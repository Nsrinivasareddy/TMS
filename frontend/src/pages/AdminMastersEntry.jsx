...
const MasterEntry = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        ope_dr: 0, ope_cr: 0,
        stationName: 'VIJAYAWADA', stationCode: 'VZA',
        allow_erp_login: false, password: ''
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const targetUrl = isAdmin 
                ? 'http://localhost:5001/api/admin/masters-save'    // 3 ఫైల్స్ సేవ్
                : 'http://localhost:5001/api/accounts/master-save';  // కేవలం masters లో సేవ్
            ...
            if (res.data.success) {
                ...
                setFormData(prev => ({
                    ...prev, input_value: '', phone: '', allow_erp_login: false, password: '', ope_dr: 0, ope_cr: 0
                }));
                mainInputRef.current?.focus();
            }
        } catch (error) {
    ...
    return (
        <div style={{ backgroundColor: '#0b1437', minHeight: '100vh', color: '#fff', fontFamily: 'monospace', padding: '20px' }}>
            ...
            <div style={{ marginTop: '20px', backgroundColor: '#111c44', padding: '25px', borderRadius: '6px', border: '1px solid #243068' }}>
                <h3 style={{ color: isAdmin ? '#ecc94b' : '#00d2ff', margin: '0 0 20px 0', fontSize: '15px' }}>
                    SRINIVASA ROAD TRANSPORT - {loginUser.login_section} MASTER ENTRY
                </h3>

                {statusMsg.text && (
                    <div style={{ padding: '10px', backgroundColor: statusMsg.type === 'success' ? '#1c4532' : '#6b2020', marginBottom: '15px', fontSize: '12px' }}>
                        {statusMsg.text}
                    </div>
                )}
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', width: '350px', gap: '5px' }}>
                        ...
                        <select name="master_type" value={formData.master_type} onChange={handleChange} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: isAdmin ? '#ecc94b' : '#00d2ff', height: '30px', fontWeight: 'bold' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', width: '450px', gap: '5px' }}>
                        <label style={{ color: '#a0aec0', fontSize: '11px' }}>
                            {isLorry ? "ENTER LORRY NUMBER (ac_name) *MANDATORY:" : "ENTER NAME / LEDGER HEAD (ac_name) *MANDATORY:"}
                        </label>
                        <input ref={mainInputRef} type="text" name="input_value" value={formData.input_value} onChange={handleChange} placeholder={isLorry ? "AP16TB1234" : "Eg: CURRENT BILLS A/C / PENALTIES A/C"} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#fff', height: '28px', padding: '0 5px' }} required />
                    </div>
                    ...
                    {isAdmin && (
                        <div style={{ display: 'flex', gap: '15px', flexDirection: 'column' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', width: '200px', gap: '5px' }}>
                                ...
                                <input type="text" name="phone" value={formData.phone} onChange={handleChange} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#fff', height: '26px', padding: '0 5px' }} />
                            </div>
                                ...
                                <div style={{ backgroundColor: '#1b254b', padding: '15px', border: '1px solid #243068', width: '500px' }}>
                                    ...
                                    {formData.allow_erp_login && (
                                        <div style={{ marginTop: '15px' }}>
                                            <input type="text" name="password" value={formData.password} onChange={handleChange} placeholder="ENTER PLAIN TEXT PASSWORD" style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#48bb78', height: '26px', padding: '0 5px', fontWeight: 'bold' }} required />
                                        </div>
                                    )}
                                </div>
                            )}
                    ...
                    {!isAdmin && (
                        <div style={{ display: 'flex', gap: '20px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', width: '120px', gap: '5px' }}>
                                ...
                                <select name="ac_type" value={formData.ac_type} onChange={handleChange} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#fff', height: '28px' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', width: '140px', gap: '5px' }}>
                                ...
                                <input type="number" name="ope_dr" value={formData.ope_dr} onChange={handleChange} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#fff', height: '26px', padding: '0 5px' }} />
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', width: '140px', gap: '5px' }}>
                                <label style={{ color: '#a0aec0', fontSize: '11px' }}>OPENING CR (Rs.):</label>
                                <input type="number" name="ope_cr" value={formData.ope_cr} onChange={handleChange} style={{ backgroundColor: '#0b1437', border: '1px solid #243068', color: '#fff', height: '26px', padding: '0 5px' }} />
                            </div>
                        </div>
                    )}

                    <div style={{ marginTop: '10px' }}>
                        <button type="submit" disabled={loading} style={{ backgroundColor: isAdmin ? '#ecc94b' : '#00b4d8', color: '#000', border: 'none', padding: '8px 25px', cursor: 'pointer', fontWeight: 'bold', fontSize: '12px' }}>
